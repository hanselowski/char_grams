...
# save W and V matrix in the folder tensors as numpy arrays
np.save('tensors/W.npy', W.detach().numpy())
np.save('tensors/V.npy', V.detach().numpy())

# save them as torch tensors
torch.save(W, 'tensors/W.pt')
torch.save(V, 'tensors/V.pt')

# also as csv files
np.savetxt('tensors/W.csv', W.detach().numpy(), delimiter=',')
np.savetxt('tensors/V.csv', V.detach().numpy(), delimiter=',')

'''
decomposition theory
W defines the bases and V are vectors in this bases
--> or rather the other way arround? 
